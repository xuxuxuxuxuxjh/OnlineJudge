# OnlineJudge 云端重新部署指南

> 本文档说明如何在本地修改代码后，将更新部署到阿里云服务器

---

## 📋 目录

1. [前端修改重新部署](#前端修改重新部署)
2. [后端修改重新部署](#后端修改重新部署)
3. [前后端同时修改](#前后端同时修改)
4. [快速部署脚本](#快速部署脚本)
5. [常见问题排查](#常见问题排查)

---

## 前端修改重新部署

### 适用场景
- 修改了 Vue 组件 (`.vue` 文件)
- 修改了前端样式 (`.css`, `.scss`)
- 修改了前端 JavaScript 代码
- 修改了前端路由或配置

### 部署步骤

#### 1. 本地构建前端

```bash
# 在 WSL 本地项目目录
cd /mnt/c/Users/wyb/Desktop/code/se/OnlineJudge/frontend

# 设置环境变量（Node 22 需要）
export NODE_OPTIONS=--openssl-legacy-provider

# 重新构建前端
yarn build

# 验证构建成功
ls -la dist/
```

**预期输出**：
```
dist/
├── admin/       # 管理后台
├── index.html   # 主页
└── static/      # 静态资源
```

#### 2. 打包前端文件

```bash
# 返回项目根目录
cd ..

# 打包前端构建结果
tar -czf /tmp/frontend-dist.tar.gz frontend/dist/
```

#### 3. 上传到服务器

```bash
# 上传到服务器
scp -i ~/.ssh/aliyun.pem /tmp/frontend-dist.tar.gz root@39.103.63.219:/tmp/

# 在服务器上解压并替换
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
rm -rf frontend/dist
mkdir -p frontend
cd frontend
tar -xzf /tmp/frontend-dist.tar.gz
rm /tmp/frontend-dist.tar.gz
echo "✅ 前端文件已更新"
EOF
```

#### 4. 重启后端容器

```bash
# 重启 backend 容器以加载新前端
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
docker compose restart oj-backend
echo "✅ 服务已重启"

# 等待服务启动
sleep 5

# 验证服务状态
docker compose ps | grep oj-backend
EOF
```

#### 5. 验证更新

在浏览器访问：`http://39.103.63.219`

按 `Ctrl + F5` 强制刷新浏览器缓存。

---

## 后端修改重新部署

### 适用场景
- 修改了 Python 代码 (`.py` 文件)
- 修改了数据库模型 (models)
- 修改了 API 接口
- 修改了配置文件

### 部署步骤

#### 1. 提交代码到 Git（推荐）

```bash
# 在本地项目根目录
cd /mnt/c/Users/wyb/Desktop/code/se/OnlineJudge

# 提交修改
git add .
git commit -m "描述你的修改"
git push
```

#### 2. 在服务器上拉取代码

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
git pull
echo "✅ 代码已更新"
EOF
```

**如果 Git 不可用，使用上传方式**：

```bash
# 打包后端代码（排除不必要的文件）
tar -czf /tmp/backend-code.tar.gz \
  --exclude='frontend/node_modules' \
  --exclude='frontend/dist' \
  --exclude='data' \
  --exclude='.git' \
  account/ announcement/ conf/ contest/ \
  judge/ oj/ options/ problem/ submission/ \
  utils/ manage.py requirements.txt

# 上传
scp -i ~/.ssh/aliyun.pem /tmp/backend-code.tar.gz root@39.103.63.219:/tmp/

# 解压
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
tar -xzf /tmp/backend-code.tar.gz
rm /tmp/backend-code.tar.gz
echo "✅ 后端代码已更新"
EOF
```

#### 3. 重新构建 Docker 镜像

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge

echo "开始构建 Docker 镜像..."
docker build -t my-onlinejudge-backend:latest .

if [ $? -eq 0 ]; then
  echo "✅ 镜像构建成功"
else
  echo "❌ 镜像构建失败"
  exit 1
fi
EOF
```

**注意**：构建可能需要 10-15 分钟。

#### 4. 运行数据库迁移（如果修改了 models）

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge

# 生成迁移文件
docker exec oj-backend python manage.py makemigrations

# 执行迁移
docker exec oj-backend python manage.py migrate

echo "✅ 数据库迁移完成"
EOF
```

#### 5. 重启服务

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge

# 停止旧容器
docker compose down

# 启动新容器
docker compose up -d

# 等待启动
sleep 30

# 检查状态
docker compose ps

echo "✅ 服务重启完成"
EOF
```

#### 6. 验证更新

```bash
# 检查后端日志
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 'docker logs oj-backend --tail 20'

# 测试 API
curl http://39.103.63.219/api/website
```

---

## 前后端同时修改

### 完整部署流程

#### 步骤 1: 本地准备

```bash
# 1. 构建前端
cd /mnt/c/Users/wyb/Desktop/code/se/OnlineJudge/frontend
export NODE_OPTIONS=--openssl-legacy-provider
yarn build

# 2. 返回根目录
cd ..

# 3. 提交代码（推荐）
git add .
git commit -m "描述你的修改"
git push
```

#### 步骤 2: 打包并上传

```bash
# 打包整个项目（如果 Git 不可用）
tar -czf /tmp/onlinejudge-update.tar.gz \
  --exclude='frontend/node_modules' \
  --exclude='node_modules' \
  --exclude='data/backend' \
  --exclude='data/judge_server' \
  --exclude='data/redis' \
  --exclude='.git' \
  --exclude='.claude' \
  .

# 上传
scp -i ~/.ssh/aliyun.pem /tmp/onlinejudge-update.tar.gz root@39.103.63.219:/tmp/
```

#### 步骤 3: 服务器端部署

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
set -e

cd /root/OnlineJudge

echo "=== 1. 停止服务 ==="
docker compose down

echo ""
echo "=== 2. 更新代码 ==="
# 如果使用 Git
git pull

# 如果使用上传
# tar -xzf /tmp/onlinejudge-update.tar.gz
# rm /tmp/onlinejudge-update.tar.gz

echo ""
echo "=== 3. 重新构建镜像 ==="
docker build -t my-onlinejudge-backend:latest .

echo ""
echo "=== 4. 启动服务 ==="
docker compose up -d

echo ""
echo "=== 5. 等待服务启动 ==="
sleep 30

echo ""
echo "=== 6. 检查服务状态 ==="
docker compose ps

echo ""
echo "=== 7. 查看日志 ==="
docker logs oj-backend --tail 10

echo ""
echo "✅ 部署完成！"
EOF
```

---

## 快速部署脚本

### 创建本地部署脚本

创建文件 `redeploy.sh`：

```bash
#!/bin/bash
# OnlineJudge 快速重新部署脚本

set -e

SERVER_IP="39.103.63.219"
SSH_KEY="~/.ssh/aliyun.pem"

echo "🚀 开始重新部署 OnlineJudge"
echo ""

# 1. 构建前端
echo "📦 步骤 1/5: 构建前端..."
cd frontend
export NODE_OPTIONS=--openssl-legacy-provider
yarn build
cd ..

# 2. 打包项目
echo "📦 步骤 2/5: 打包项目..."
tar -czf /tmp/oj-update.tar.gz \
  --exclude='frontend/node_modules' \
  --exclude='node_modules' \
  --exclude='data' \
  --exclude='.git' \
  --exclude='.claude' \
  .

# 3. 上传到服务器
echo "📤 步骤 3/5: 上传到服务器..."
scp -i $SSH_KEY /tmp/oj-update.tar.gz root@$SERVER_IP:/tmp/

# 4. 服务器端部署
echo "🔧 步骤 4/5: 服务器端部署..."
ssh -i $SSH_KEY root@$SERVER_IP << 'ENDSSH'
cd /root/OnlineJudge
docker compose down
tar -xzf /tmp/oj-update.tar.gz
rm /tmp/oj-update.tar.gz
docker build -t my-onlinejudge-backend:latest .
docker compose up -d
sleep 20
docker compose ps
ENDSSH

# 5. 测试访问
echo "✅ 步骤 5/5: 测试访问..."
sleep 5
curl -s -o /dev/null -w "HTTP: %{http_code}\n" http://$SERVER_IP

echo ""
echo "🎉 部署完成！访问: http://$SERVER_IP"
```

**使用方法**：

```bash
# 赋予执行权限
chmod +x redeploy.sh

# 运行部署
./redeploy.sh
```

---

## 常见问题排查

### 问题 1: 前端修改未生效

**原因**：浏览器缓存

**解决方案**：
```bash
# 1. 强制刷新浏览器（Ctrl + F5）
# 2. 清除浏览器缓存
# 3. 使用隐身模式访问
```

### 问题 2: 后端容器启动失败

**检查日志**：
```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 'docker logs oj-backend --tail 50'
```

**常见原因**：
- Python 语法错误
- 缺少依赖包
- 数据库迁移未执行

**解决方案**：
```bash
# 进入容器调试
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219
docker exec -it oj-backend sh
python manage.py check
```

### 问题 3: Docker 镜像构建失败

**查看完整构建日志**：
```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
docker build -t my-onlinejudge-backend:latest . 2>&1 | tee build.log
EOF
```

### 问题 4: 数据库迁移失败

**解决方案**：
```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
docker exec oj-backend python manage.py showmigrations
docker exec oj-backend python manage.py migrate --fake-initial
EOF
```

### 问题 5: 端口冲突

**检查端口占用**：
```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 'netstat -tulpn | grep :80'
```

**重启 Docker**：
```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
docker compose down
docker compose up -d
EOF
```

---

## 📝 部署检查清单

部署后确认以下事项：

- [ ] 所有容器状态为 `Up (healthy)`
- [ ] 前端页面可以正常访问
- [ ] 管理后台可以登录
- [ ] API 接口正常响应
- [ ] 判题功能正常（提交代码测试）
- [ ] 查看日志无错误信息

---

## 🔄 回滚操作

如果部署后出现问题，可以快速回滚：

### 方法 1: 使用 Git 回滚

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
cd /root/OnlineJudge
git log --oneline -5           # 查看最近提交
git reset --hard <commit-id>   # 回滚到指定版本
docker compose down
docker build -t my-onlinejudge-backend:latest .
docker compose up -d
EOF
```

### 方法 2: 使用 Docker 镜像回滚

```bash
ssh -i ~/.ssh/aliyun.pem root@39.103.63.219 << 'EOF'
docker images | grep onlinejudge   # 查看历史镜像
docker tag <old-image-id> my-onlinejudge-backend:latest
docker compose up -d
EOF
```

---

## 🚀 性能优化建议

### 1. 使用 CI/CD 自动部署

可以配置 GitHub Actions 自动部署：

```yaml
# .github/workflows/deploy.yml
name: Deploy to Aliyun

on:
  push:
    branches: [ main ]

jobs:
  deploy:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v2
      - name: Deploy
        run: |
          # 部署脚本
```

### 2. 增量更新

只上传修改的文件，而不是整个项目。

### 3. 使用私有 Docker Registry

将镜像推送到阿里云容器镜像服务，加快部署速度。

---

## 📞 需要帮助？

- 查看日志：`docker logs oj-backend`
- 查看系统状态：`docker compose ps`
- 查看资源使用：`docker stats`

---

**最后更新**: 2025-11-17
**适用版本**: OnlineJudge 2.0
